    ...
    void Xmax(const Parabola *AptDatos,Result *AptRes)
         {
                      AptRes->Xmax=(pow(AptDatos->Vel,2)*sin(2*AptRes->Grados))/(AptDatos->Grav);
         }
         
    void Ymax(const Parabola *AptDatos, Result *AptRes)
         {
                       AptRes->Ymax=(pow(AptDatos->Vel,2)*pow(sin(AptRes->Grados),2))/(2*AptDatos->Grav);
         }
         
    void Time(const Parabola *AptDatos, Result *AptRes)
         {
                       AptRes->Time=(AptDatos->Vel*sin(AptRes->Grados))/(AptDatos->Grav);
         }
